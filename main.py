# -*- coding: utf-8 -*-
"""
Created on Mon Aug 16 22:22:12 2021
...
async def run(client, debug=False):
    ...
    plt.ylabel("Voltage in microvolts", fontsize=15)
    plt.xlabel(
        "sample no.",
        fontsize=10,
    )
    ...
    while time.time() - start_time < 10:
        ...
        print(len(ecg_session_data))
        print(len(ecg_session_time))
        ax.plot(ecg_session_data, color="r")
        fig.canvas.draw()
        ax.set_xlim(left=n - 130, right=n)
        plt.pause(0.05)
        n = n + 130
        all_data += [ecg_session_data]
        x += [n]
